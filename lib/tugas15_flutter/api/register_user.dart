...
import 'package:ppkd_b_3/tugas15_flutter/api/endpoint/endpoint.dart';
...
class AuthenticationAPI {
  static Future<RegisterUserModel> registerUser({
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception(
        error["message"]?.toString() ?? "Register gagal",
      ); // Pastikan String
    }
  }
  ...
  static Future<RegisterUserModel> loginUser({
    ...
    if (response.statusCode == 200) {
      ...
      final error = json.decode(response.body);
      throw Exception(error["message"]?.toString() ?? "Login gagal");
    }
  }
  ...
  static Future<GetUserModel> updateUser({required String name}) async {
    ...
    final response = await http.put(
      ...
      headers: {
        ...
        "Authorization": "Bearer $token", // Tambahkan Bearer jika diperlukan
      },
    );
    print(response.body);
  ...
  static Future<GetUserModel> getProfile() async {
    ...
    if (response.statusCode == 200) {
      ...
      final error = json.decode(response.body);
      // Pastikan mengembalikan String, bukan bool atau tipe lainnya
      throw Exception(error["message"]?.toString() ?? "Gagal mengambil profil");
    }
  }
}
